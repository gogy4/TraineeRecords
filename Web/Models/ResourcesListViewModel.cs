...
public class ResourcesListViewModel
{
    ...
    public ResourcesListViewModel(string activeTab, string searchQuery, string sortOrder, int totalPagesProjects,
        int totalPagesDirections, List<CurrentProjectDto> projects, List<InternshipDirectionDto> directions,
        Dictionary<Guid, List<TraineeDto>> traineesByProject, Dictionary<Guid, List<TraineeDto>> traineesByDirection,
        string success, string errors, int currentPage = 1, int pageSize = 10)
    {
        SortOrder = sortOrder;
        ActiveTab = activeTab;
